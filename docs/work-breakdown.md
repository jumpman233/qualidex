...
- P0-5 多人员文件识别已完成：AI 标记多人员资料后，文件会建立到所有识别人员的 `person_documents` 关联；归档预览和归档写入对多人员文件按文件去重，只复制到 `_多人员资料`；查询资料文件夹导出也使用 `_多人员资料`，不会复制进每个人目录。
- P0-6 人工校验可查看文件已完成：待确认卡片可展示原始路径、来源文件夹、相对路径、OCR 文本、AI 结果、路径解析和文件夹归并证据；支持从主进程按待确认项打开原始文件或所在文件夹；字段修改、人员切换、新建人员、确认/忽略保持审计记录。
- P0-7 导入完成 CTA 已完成：导入处理队列完成且无剩余任务时，页面根据真实待确认数量引导到“查看待确认资料”或“生成归档预览”；重复文件仍显示为跳过且不创建处理任务。
- P0-8 路径语义解析已完成：导入时基于相对路径、父文件夹和文件名生成 `path_parse_result/path_confidence`，提取主类别、地区、人员、资料类型和证书线索；AI 输入会携带脱敏后的路径语义提示；结构化识别会将路径候选与识别结果做冲突比对并生成路径冲突待确认项。
