...
- [P0] 每条证书独立生成 `license_search_text`。
- [P0] 同一人员可以拥有多条 `licenses` 记录。
- [P0] 证书查询命中任一有效证书时，该人员可进入候选结果。
- [P1] 查询结果和导出需要展示该人员的全部匹配证书，以及必要的非匹配证书摘要。
...
- [P0] `sqlite-vec` 召回的是证书候选，不是人员候选。
- [P0] 用户确认匹配证书后，SQL 通过 `licenses.person_id` 查询人员。
- [P0] 一个人有多张证书时，任一证书满足条件即可匹配该证书条件。
- [P1] 多证书同时满足多个条件时，结果应标明每个条件命中的证书。
...
字段：类别、地区、学历、证书、证书状态、证书官方/非官方标签、证书有效期、资料完整度、是否包含待确认、是否包含多人员资料、导入批次

### 14.4 查询结果字段

选择、姓名、主类别、地区、学历、身份证号脱敏值、证书、证书状态、证书官方/非官方标签、证书有效期、资料完整度、是否待确认、是否包含多人员资料、文件夹路径、匹配原因、操作
...
- [P0] 地区
- [P0] 身份证号脱敏值
- [P1] 完整身份证号，受 `export_full_id_card` 开关控制，默认不导出
- [P0] 学历
- [P0] 证书名称
- [P0] 证书状态
- [P1] 证书官方/非官方标签
...
- 默认导出脱敏值。
- 用户明确选择 `export_full_id_card = true` 后，才导出完整身份证号。
- 导出日志需要记录是否启用了完整身份证号导出。

多证书导出规则：

- 一人多证书时，可按“一人一行，证书合并展示”或“一人多行，每证书一行”导出。
- MVP 默认可采用证书合并展示，证书之间用分隔符区分。
- 如果查询条件命中了多张证书，匹配原因中应列出命中的证书。
...
- 导出目录和人员资料文件夹不暴露完整身份证号。
- 文件夹命名仍使用姓名 + 身份证后四位或系统编号。
- 多人员资料不自动复制到每个人目录，应保留多人员提示和引用路径。
